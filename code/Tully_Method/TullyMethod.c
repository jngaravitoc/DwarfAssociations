	...
	n=0;
        for(j=0;j<n_points;j++){
	      ...
	      n++;
              ...
              printf("%f \t %d \t %d \t %d \t %f \t %f \t %f \t %f \t %f \t %f \t %d \t %d \n", F[j], p, q, j, m[p], m[q], mt[j], pow(D[j+p*n_points], 2), pow(D[j+q*n_points], 2), pow(D[q+p*n_points], 2), q+p*n_points, p+q*n_points);
	      /*if(n>0){
